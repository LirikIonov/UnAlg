...
public class F {
    ...
    void generateRandomBinKey(int len) {
    ...
    void createCryptedKey() {
        StringBuilder tempKey = new StringBuilder();
        StringBuilder keySb = new StringBuilder(key);
        int len  = keySb.length();
        keySb.reverse();
        int i;
        for (i = 0; i < len; i += 2) {
            if (i < len - 1) {
                tempKey.append((keySb.charAt(i) - '0') |
                        (keySb.charAt(i + 1) - '0'));
            }
        }
        if (len % 2 == 1) {
            tempKey.append(Integer.valueOf(keySb.charAt(len - 1) - '0'));
        }
        cryptKey = tempKey.reverse().toString();
    }

    void computations() throws Exception {
        // Read the input.txt
        createInput("inputRu.txt");
        ...
        alph += " 0123456789 .,?:;!@#$%^*()_+-=—&[]";
        alphLen = alph.length();
        generateRandomBinKey(limit.length());
        createCryptedKey();
    ...
    String solve(int ch) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char l = s.charAt(i);
            char r = (cryptKey.charAt(i % cryptKey.length()));
            res.append(l ^ r);
        }
        // 2
